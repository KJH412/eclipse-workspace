...
public class EmpDAO {
	...
	Statement st;
	ResultSet rs; 
	PreparedStatement pst; //바인딩 변수('?')를 지원
	
	//로그인하기
	public EmpDTO loginCheck(String email, String phone) { 
		EmpDTO emp = null; 
		String sql = "select employee_id, first_name, phone_number, last_name, phone_number"
		...
		try {
			...
			if(rs.next()) {
				...
				if(rs.getString("phone_number").equals(phone)) {
					...
					//순서가 아닌 이름으로 넣어서 그냥 추가하면 된다. (boardInsert.do)
					emp.setLast_name(rs.getString("last_name")); //sql문 select 칼럼에 추가
					emp.setEmail(email);
	...
	//전체 JOB 조회 (테이블JOBS 칼럼값 가져오기 위해 JobDTO만들고 객체생성.)
